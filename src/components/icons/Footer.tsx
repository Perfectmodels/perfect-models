    ...
    <footer className="overflow-hidden border-t border-white/10 bg-pm-ink text-pm-ivory">
      <div className="mx-auto max-w-[1600px] px-5 pb-10 pt-20 sm:px-8 sm:pt-28 lg:px-12">
        ...
        <div className="grid gap-12 py-14 sm:grid-cols-2 lg:grid-cols-[1.2fr_.8fr_1fr] lg:py-20">
          <div>
            <Link href="/" className="inline-flex items-center gap-4"><span className="grid h-14 w-14 place-items-center border border-pm-gold font-playfair text-2xl text-pm-gold">PM</span><span><span className="block font-playfair text-2xl">Perfect Models</span><span className="mt-1 block text-[8px] font-bold uppercase tracking-[.32em] text-white/35">Management · Libreville</span></span></Link>
          ...
          <div>
            ...
            <nav className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-1">
              {navLinks.slice(0, 7).map((link) => <Link key={link.path} href={link.path} className="font-playfair text-xl text-white/60 transition hover:translate-x-1 hover:text-pm-ivory">{link.footerLabel || link.label}</Link>)}
            </nav>
          </div>
        ...
        <div className="flex flex-col gap-4 border-t border-white/10 pt-7 text-[8px] font-bold uppercase tracking-[.25em] text-white/25 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-6"><Link href="/privacy-policy" className="transition hover:text-pm-gold">Confidentialité</Link><Link href="/terms-of-use" className="transition hover:text-pm-gold">Conditions</Link><Link href="/login" className="text-pm-gold/70 transition hover:text-pm-gold">Espace privé</Link></div>
        </div>
