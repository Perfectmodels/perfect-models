...
const Footer: React.FC<{ runtimeData?: RuntimeData | null }> = ({ runtimeData }) => {
  ...
  return (
    <footer className="overflow-hidden border-t border-pm-wine/10 bg-pm-wine text-pm-ivory">
      <div className="mx-auto max-w-[1700px] px-5 pb-9 pt-20 sm:px-8 sm:pt-28 lg:px-12 xl:px-16">
        <div className="grid gap-12 border-b border-white/20 pb-16 lg:grid-cols-[1.3fr_.7fr] lg:items-end lg:pb-20">
          <div>
            ...
            <h2 className="mt-7 max-w-5xl font-playfair text-[clamp(4rem,8vw,8.2rem)] font-semibold leading-[.82] tracking-[-.06em]">Un talent.<br /><em className="font-normal text-pm-peach">Une vision.</em></h2>
          </div>
          <div className="grid gap-3 lg:pb-2">
            <Link href="/contact?subject=booking" className="pmm-button rounded-full border-pm-ivory bg-pm-ivory text-pm-wine hover:border-pm-gold-light hover:bg-pm-gold-light">Booker un talent <span>↗</span></Link>
            <Link href="/casting-formulaire" className="pmm-button rounded-full border-white/45 text-white hover:border-white hover:bg-white hover:text-pm-wine">Rejoindre l’agence</Link>
          </div>
        </div>
        ...
        <div className="grid gap-12 py-14 sm:grid-cols-2 lg:grid-cols-[1.15fr_.75fr_1.1fr] lg:py-20">
          <div>
            <Link href="/" className="inline-flex items-center gap-4">
              <span className="h-14 w-14 overflow-hidden rounded-full border border-white/25 bg-white"><img src="/logopmm.jpg" alt="" className="h-full w-full object-cover" /></span>
              <span>
                <span className="block font-playfair text-2xl font-semibold tracking-[-.02em]">Perfect Models</span>
                <span className="mt-1.5 block text-[8px] font-black uppercase tracking-[.32em] text-white/55">Management · Gabon</span>
              </span>
            </Link>
            <p className="mt-7 max-w-sm text-sm leading-7 text-white/68">Agence de management, casting, formation et production dédiée au développement des talents et de l’image de mode au Gabon.</p>
          </div>
          ...
          <div>
            ...
            <nav className="grid grid-cols-2 gap-x-8 gap-y-3 sm:grid-cols-1">
              {navLinks.slice(0, 7).map((link) => <Link key={link.path} href={link.path} className="font-playfair text-xl text-white/75 transition duration-300 hover:translate-x-1 hover:text-pm-gold-light">{link.footerLabel || link.label}</Link>)}
            </nav>
          </div>
          ...
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {socials.map(([label, href, Icon]) => href ? <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="grid h-10 w-10 place-items-center rounded-full border border-white/25 text-white/65 transition duration-300 hover:border-pm-gold-light hover:bg-pm-gold-light hover:text-pm-wine"><Icon className="h-4 w-4" /></a> : null)}
            </div>
          </div>
        </div>

        <div className="flex flex-col gap-4 border-t border-white/20 pt-7 text-[8px] font-black uppercase tracking-[.23em] text-white/50 sm:flex-row sm:items-center sm:justify-between">
        ...
        <p aria-hidden="true" className="pointer-events-none -mb-[3.5vw] mt-12 whitespace-nowrap text-center font-playfair text-[15vw] font-semibold leading-[.72] tracking-[-.075em] text-white/[.06]">PERFECT</p>
      </div>
    </footer>
  );
};
