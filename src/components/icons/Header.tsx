...
export const Breadcrumb: React.FC = () => {
  ...
  return (
    <nav aria-label="Fil d’Ariane" className="mx-auto flex max-w-[1600px] items-center gap-2 px-5 py-4 text-[8px] font-bold uppercase tracking-[.28em] text-white/25 sm:px-8 lg:px-12">
      ...
      {crumbs.map((crumb, index) => <React.Fragment key={crumb.path}><span aria-hidden="true">/</span>{index === crumbs.length - 1 ? <span className="text-white/55">{crumb.label}</span> : <Link href={crumb.path} className="transition hover:text-pm-gold">{crumb.label}</Link>}</React.Fragment>)}
    </nav>
  );
};
      ...
      <header className={`sticky top-0 z-50 border-b transition duration-300 ${scrolled ? 'border-white/10 bg-pm-dark/95 shadow-2xl shadow-black/20 backdrop-blur-xl' : 'border-white/10 bg-pm-dark'}`}>
        <div className="mx-auto flex h-[78px] max-w-[1600px] items-center px-5 sm:px-8 lg:px-12">
          ...
          <nav className="ml-auto hidden items-center gap-5 xl:flex 2xl:gap-7">
            {NAV_LINKS.map((link) => <Link key={link.path} href={link.path} className={`relative py-2 text-[9px] font-bold uppercase tracking-[.2em] transition ${active(link.path) ? 'text-pm-gold' : 'text-white/52 hover:text-white'}`}><span>{link.label}</span>{active(link.path) && <span className="absolute inset-x-0 -bottom-[1.38rem] h-px bg-pm-gold" />}</Link>)}
          </nav>

          <div className="ml-auto flex items-center xl:ml-7">
            <Link href="/login" className="hidden border-l border-white/10 px-5 py-2 text-[8px] font-bold uppercase tracking-[.22em] text-white/45 transition hover:text-pm-gold sm:block">Espace privé</Link>
            <Link href="/casting-formulaire" className="hidden border border-pm-gold bg-pm-gold px-5 py-3 text-[8px] font-black uppercase tracking-[.22em] text-pm-dark transition hover:bg-pm-ivory lg:inline-flex">Candidater ↗</Link>
