...
import { isDirectImgBBImageUrl } from '@/utils/imgbbService';
...
export default function MediaPicker({ isOpen, onClose, onSelect, multiple = false, resourceType = 'auto', title = 'Bibliothèque de médias' }: MediaPickerProps) {
  ...
  const filtered = useMemo(() => gallery.filter(item => {
    ...
    const trustedImage = mediaType !== 'image' || (isDirectImgBBImageUrl(item.url) && !failedImages.has(item.url));
    return trustedImage
      && (activeCategory === 'Tout' || category === activeCategory)
      && (resourceType === 'auto' || mediaType === resourceType)
      && (!search || caption.toLowerCase().includes(search.toLowerCase()));
  }), [activeCategory, failedImages, gallery, resourceType, search]);

  if (!isOpen) return null;

  const toggleSelection = (item: MediaRow) => {
    const isVideo = String(item.mime_type || '').toLowerCase().startsWith('video');
    if (!isVideo && (!isDirectImgBBImageUrl(item.url) || failedImages.has(item.url))) return;
  ...
  const confirm = () => { onSelect(Array.from(selected).filter((url) => !failedImages.has(url))); onClose(); };
  ...
  return <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/90 p-4 backdrop-blur-md" onClick={onClose}>
    <div className="flex max-h-[90vh] w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-pm-gold/20 bg-[#0d0d0d] shadow-2xl" onClick={event => event.stopPropagation()}>
      <div className="flex items-center justify-between border-b border-white/5 bg-pm-dark/50 px-6 py-5"><div className="flex items-center gap-3"><div className="flex h-9 w-9 items-center justify-center rounded-full bg-pm-gold/10"><PhotoIcon className="h-5 w-5 text-pm-gold"/></div><div><h2 className="font-playfair text-base font-black text-white">{title}</h2><p className="text-[10px] uppercase tracking-widest text-white/30">{multiple ? `${selected.size} média(s) sélectionné(s)` : 'Sélectionner un média vérifié'}</p></div></div><button onClick={onClose} className="flex h-8 w-8 items-center justify-center rounded-full text-white/30 hover:bg-white/5 hover:text-white"><XMarkIcon className="h-5 w-5"/></button></div>
      ...
      <div className="flex-1 overflow-y-auto p-6">{isLoading ? <div className="grid min-h-72 place-items-center text-sm text-white/35">Chargement depuis Supabase…</div> : filtered.length === 0 ? <div className="flex min-h-72 flex-col items-center justify-center gap-4 text-white/20"><PhotoIcon className="h-16 w-16"/><p className="text-sm font-black uppercase tracking-widest">Aucun média ImgBB disponible</p></div> : <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">{filtered.map(item => {
        const isVideo = String(item.mime_type || '').toLowerCase().startsWith('video');
        const caption = String(item.alt_text || item.file_name || item.metadata?.caption || '');
        const thumbnail = String(item.metadata?.thumbnailUrl || item.metadata?.thumbnail_url || '');
        return <button type="button" key={item.id} onClick={() => toggleSelection(item)} className={`group relative aspect-square overflow-hidden rounded-xl border-2 text-left transition ${selected.has(item.url) ? 'scale-[0.98] border-pm-gold' : 'border-transparent hover:border-white/20'}`}>{isVideo ? <div className="h-full w-full bg-pm-dark">{thumbnail ? <img src={thumbnail} alt="" className="h-full w-full object-cover"/> : <div className="grid h-full w-full place-items-center"><VideoCameraIcon className="h-8 w-8 text-white/20"/></div>}<div className="absolute inset-0 grid place-items-center"><VideoCameraIcon className="h-6 w-6 text-white"/></div></div> : <img src={item.url} alt={caption} className="h-full w-full object-cover" onError={() => { setFailedImages(previous => new Set(previous).add(item.url)); setSelected(previous => { const next = new Set(previous); next.delete(item.url); return next; }); }}/>} {selected.has(item.url) && <CheckCircleIcon className="absolute right-2 top-2 h-6 w-6 fill-pm-dark text-pm-gold"/>}{caption && <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent p-2"><p className="truncate text-[10px] text-white">{caption}</p></div>}</button>;
      })}</div>}</div>
