...
function compactValue(value: unknown, column: string, fields: readonly CrudField[] = []) {
  ...
  if (field?.type === 'number' && /(amount|total|fee|rate|budget|income|expense|price|cost)/i.test(column) && value !== null && value !== undefined && value !== '') {
    const amount = Number(value);
    if (Number.isFinite(amount)) return <span className="font-semibold">{new Intl.NumberFormat('fr-FR',{style:'currency',currency:'XAF',maximumFractionDigits:0}).format(amount)}</span>;
  }
...
export default function ResponsiveResourceManager({ resource, title, primaryKey, columns, fields, initialRows, initialTotal, canCreate = true, canDelete = true }: Props) {
  ...
  const archiveValue = useMemo(() => {
    ...
    return ['archived','inactive','closed'].find((value) => values.includes(value)) || '';
  }, [statusOptions]);
  ...
  const archive = async () => {
    ...
    try {
      ...
      setArchiving(null); setNotice('Dossier archivé.'); await loadRows();
