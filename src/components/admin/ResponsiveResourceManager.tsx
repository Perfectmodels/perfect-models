...
function isImageUrlField(name: string) {
  return /(^|_)(image|photo|logo|avatar|thumbnail)_url$/i.test(name) || ['image_url', 'cover_image_url', 'logo_url'].includes(name);
}
...
function relationLabel(value: unknown, column: string, fields: readonly CrudField[]) {
  const field = fields.find((item) => item.name === column);
  return field?.options?.find((option) => String(option.value) === String(value))?.label || '';
}

function displayTitle(row: Row, primaryKey: string, fields: readonly CrudField[]) {
  for (const key of ['name','title','full_name','display_name','subject','campaign','quote_number','invoice_number','email']) {
    if (row[key]) return String(row[key]);
  }
  for (const field of fields) {
    if (!field.name.endsWith('_id') || !row[field.name]) continue;
    const label = relationLabel(row[field.name], field.name, fields);
    if (label) return label;
  }
  return row[primaryKey] ? 'Dossier enregistré' : 'Dossier';
}

function compactValue(value: unknown, column: string, fields: readonly CrudField[] = [], row?: Row) {
  const field = fields.find((item) => item.name === column);
  const optionLabel = relationLabel(value, column, fields);
  if (optionLabel) value = optionLabel;
  if (field?.type === 'number' && /rate|percent|percentage/i.test(column) && value !== null && value !== undefined && value !== '') {
    const rate = Number(value);
    if (Number.isFinite(rate)) return <span className="font-semibold">{new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 2 }).format(rate)} %</span>;
  }
  if (field?.type === 'number' && /(amount|total|fee|budget|income|expense|price|cost)/i.test(column) && value !== null && value !== undefined && value !== '') {
    const amount = Number(value);
    const currency = typeof row?.currency === 'string' && row.currency ? row.currency : 'XAF';
...
function FieldInput({ resource, field, value, onChange, editing, relationOptions, inputId, describedBy }: {
  resource: string;
  field: CrudField;
  value: FormValue;
  ...
  if (isImageUrlField(field.name)) {
    return <div id={inputId} aria-describedby={describedBy} className={disabled ? 'pointer-events-none opacity-55' : ''}><ImgBBUploader value={String(value || '')} onChange={onChange} scope={`admin/${resource}/${field.name}`} compact /></div>;
  }
  if (field.type === 'json' && IMAGE_LIST_FIELDS.has(field.name)) {
    let urls: string[] = [];
    try { const parsed = JSON.parse(String(value || '[]')); if (Array.isArray(parsed)) urls = parsed.filter((item): item is string => typeof item === 'string'); } catch { urls = []; }
    return <div id={inputId} aria-describedby={describedBy} className={disabled ? 'pointer-events-none opacity-55' : ''}><ImgBBMultiUploader values={urls} onChange={(next) => onChange(JSON.stringify(next))} scope={`admin/${resource}/${field.name}`} /></div>;
  }
  if (field.type === 'json') {
    return <div id={inputId} aria-describedby={describedBy}><StructuredDataField fieldName={field.name} value={String(value || '{}')} onChange={onChange} disabled={disabled} resource={resource} /></div>;
  }
  ...
  if (field.type === 'select' || options.length) {
    ...
    const normalized = options.some((option) => String(option.value) === current) || !current
      ? options
      : [{ label: field.name.endsWith('_id') ? 'Dossier actuellement lié' : current, value: current }, ...options];
...
export default function ResponsiveResourceManager({ resource, title, primaryKey, columns, fields, initialRows, initialTotal, canCreate = true, canDelete = true }: Props) {
  ...
  useEffect(() => {
    if (!editor && !viewing && !deleting && !archiving) return;
    const onKey = (event: KeyboardEvent) => {
      if (event.key !== 'Escape' || busy) return;
      setEditor(null); setViewing(null); setDeleting(null); setArchiving(null); setError('');
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, [editor, viewing, deleting, archiving, busy]);
  ...
  const archiveValue = useMemo(() => {
    const values = statusOptions.map((option) => String(option.value));
    return ['archived','inactive','closed','cancelled'].find((value) => values.includes(value)) || '';
  }, [statusOptions]);
  const archiveLabel = archiveValue === 'cancelled' ? 'Annuler' : archiveValue === 'closed' ? 'Clôturer' : 'Archiver';
  const archiveDoneLabel = archiveValue === 'cancelled' ? 'Dossier annulé.' : archiveValue === 'closed' ? 'Dossier clôturé.' : 'Dossier archivé.';

  const archive = async () => {
    if (!archiving || !archiveValue) return;
    const id = archiving[primaryKey]; if (id === undefined || id === null) return;
    setBusy(true); setError(''); setNotice('');
    try {
      const response = await fetch(`/api/admin/resources/${encodeURIComponent(resource)}/${encodeURIComponent(String(id))}`, { method:'PATCH', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify({status:archiveValue}) });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Archivage impossible.');
      setArchiving(null); setNotice(archiveDoneLabel); await loadRows();
