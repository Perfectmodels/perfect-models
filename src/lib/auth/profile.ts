...
function normalizeAppRole(value: unknown, fallback: AppRole = 'student'): AppRole {
  const map: Record<string, AppRole> = {
    admin: 'admin', manager: 'manager', gestionnaire: 'manager',
    student: 'student', mannequin: 'student', modele: 'student',
    jury: 'jury', registration: 'registration', 'jury-contest': 'jury-contest',
    accueil: 'registration', staff: 'registration',
  };
...
export async function findProfile(user: unknown, idToken?: string | null): Promise<AppSessionProfile | null> {
  ...
  if (model) {
    const modelRole = normalizeAppRole(model.role, 'student');
    const permissions = model.permissions && typeof model.permissions === 'object' ? model.permissions as Record<string, boolean> : { isActive: true };
    const role: AppRole = modelRole === 'manager' ? 'manager' : (permissions.isAdmin === true || model.adminPermissions !== undefined ? 'admin' : 'student');
    return {
