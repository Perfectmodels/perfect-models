...
function parseJson(field: CrudField, value: unknown) {
  if (typeof value !== 'string') {
    ...
    throw new CrudValidationError(`${field.label} doit contenir un objet ou une liste JSON valide.`);
  }
  try {
    return JSON.parse(value);
  } catch {
    throw new CrudValidationError(`${field.label} contient un JSON invalide.`);
  }
}
...
function normalize(field: CrudField, value: unknown) {
  ...
  if (field.type === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(text)) {
    throw new CrudValidationError(`${field.label} n’est pas une adresse e-mail valide.`);
  }
  if (field.type === 'url') {
    try {
      ...
      throw new CrudValidationError(`${field.label} doit être une URL complète commençant par http:// ou https://.`);
    }
  }
  return text;
}
...
export function sanitizeResourcePayload(resource: ResourceName, input: unknown, mode: Mode) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new CrudValidationError('Le formulaire envoyé est invalide.');
  }
  ...
  for (const field of definition.fields) {
    ...
    const hasValue = Object.prototype.hasOwnProperty.call(source, field.name);
