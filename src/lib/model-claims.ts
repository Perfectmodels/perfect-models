...
export function verificationMode(model: any, input: { email: string; phone: string; birthDate: string }): 'automatic' | 'agency_review' {
  const storedEmail = String(model?.email || '').trim().toLowerCase();
  const storedPhone = normalizePhone(model?.phone);
  const storedBirth = String(model?.birth_date || '').slice(0, 10);
  const emailMatch = Boolean(storedEmail && storedEmail === input.email.toLowerCase());
  const phoneMatch = Boolean(storedPhone && storedPhone === normalizePhone(input.phone));
  const birthMatch = Boolean(storedBirth && storedBirth === input.birthDate);
...
export async function activateModelClaim(model: any, authUser: any) {
  ...
  const pending = objectValue(raw.pendingClaim) as PendingModelClaim;
  ...
  const permissions = objectValue(model.permissions);
  const resolvedPermissions = Object.keys(permissions).length ? permissions : { canAccessFormation: true, canAccessClassroom: true, canAccessForum: true, canViewPhotoshootBriefs: true, canViewResults: true, canEditProfile: true, isActive: true };
  const birth = pending.birthDate || null;
  const age = birth ? Math.max(0, new Date().getFullYear() - new Date(birth).getFullYear()) : model.age;
  ...
  const { data: linked, error: linkError } = await supabase.from('models').update({
    ...
    gender: pending.gender || model.gender, nationality: pending.nationality || model.nationality,
    location: pending.city || model.location, height: pending.heightCm ? `${pending.heightCm}cm` : model.height,
