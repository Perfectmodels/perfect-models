...
const ERP_RESOURCE_DEFINITIONS = {
  'model-collaborations': r('model_collaborations', 'Collaborations du talent', 'collaboration_date',
    ['collaborator_name', 'project_title', 'collaboration_type', 'collaboration_date', 'location', 'is_public'],
    [relation('model_id', 'Mannequin', true), f('collaborator_name', 'Client, marque ou partenaire', 'text', { required: true }), f('project_title', 'Projet'), f('collaboration_type', 'Type de collaboration', 'select', { options: [option('Shooting', 'shooting'), option('Défilé', 'runway'), option('Campagne', 'campaign'), option('Presse', 'press'), option('Événement', 'event'), option('Autre', 'other')] }), date('collaboration_date', 'Date'), f('date_label', 'Date libre'), f('location', 'Lieu'), textarea('description', 'Description'), f('images', 'Images', 'json', { defaultValue: [] }), f('is_public', 'Visible sur le profil public', 'boolean', { defaultValue: true }), f('position', 'Ordre', 'number', { min: 0, step: 1, defaultValue: 0 })]),

  'model-events': r('model_events', 'Palmarès, shootings & défilés', 'event_date',
    ['name', 'event_type', 'event_date', 'role', 'designer', 'location', 'is_public'],
    [relation('model_id', 'Mannequin', true), f('name', 'Nom du projet ou événement', 'text', { required: true }), f('event_type', 'Type', 'select', { options: [option('Défilé', 'runway'), option('Shooting', 'shooting'), option('Campagne', 'campaign'), option('Presse', 'press'), option('Casting', 'casting'), option('Autre', 'other')] }), date('event_date', 'Date'), f('date_label', 'Date libre'), f('role', 'Rôle / participation'), f('designer', 'Créateur / designer'), f('organizer', 'Organisateur'), f('location', 'Lieu'), textarea('description', 'Description'), f('images', 'Images', 'json', { defaultValue: [] }), f('is_public', 'Visible sur le profil public', 'boolean', { defaultValue: true }), f('position', 'Ordre', 'number', { min: 0, step: 1, defaultValue: 0 })]),
  ...
  payments: r('monthly_payments', 'Cotisations & paiements talents', 'created_at',
    ['model_id', 'transaction_type', 'period', 'amount', 'payment_method', 'reference', 'status', 'paid_at'],
    [relation('model_id', 'Mannequin', true), f('transaction_type', 'Objet du paiement', 'select', { required: true, options: [option('Pack adhésion', 'membership_package'), option('Inscription', 'registration'), option('Cotisation mensuelle', 'membership_fee'), option('Commission agence', 'agency_commission'), option('Autre paiement', 'other')], defaultValue: 'membership_fee' }), date('period', 'Période', true), f('amount', 'Montant', 'number', { required: true, min: 0.01, step: 0.01 }), f('currency', 'Devise', 'select', { options: [option('Franc CFA', 'XAF'), option('Euro', 'EUR'), option('Dollar US', 'USD')], defaultValue: 'XAF' }), f('payment_method', 'Moyen de paiement', 'select', { options: [option('Saisie administrative', 'manual'), option('Espèces', 'cash'), option('Airtel Money', 'airtel_money'), option('Moov Money', 'moov_money'), option('Virement bancaire', 'bank_transfer'), option('Autre', 'other')] }), f('reference', 'Référence'), f('proof_url', 'Preuve de paiement', 'url'), f('status', 'Statut', 'select', { options: status(['À vérifier', 'pending'], ['Confirmé', 'validated'], ['Rejeté', 'rejected']), defaultValue: 'pending' }), datetime('paid_at', 'Payé le'), textarea('validation_notes', 'Note administrative')],
    { canDelete: false }),
  ...
  messages: r('messages', 'Communications CRM', 'created_at',
    ['direction', 'channel', 'client_id', 'recipient', 'subject', 'status', 'created_at'],
    [f('direction', 'Sens', 'select', { required: true, options: [option('Entrant', 'inbound'), option('Sortant', 'outbound'), option('Interne', 'internal')], defaultValue: 'outbound' }), f('channel', 'Canal', 'select', { required: true, options: [option('E-mail', 'email'), option('SMS', 'sms'), option('WhatsApp', 'whatsapp'), option('Téléphone', 'phone'), option('Note interne', 'internal')] }), relation('client_id', 'Client'), relation('casting_id', 'Casting'), relation('booking_id', 'Booking'), relation('invoice_id', 'Facture'), relation('model_id', 'Mannequin'), f('recipient', 'Destinataire'), f('sender', 'Expéditeur'), f('subject', 'Objet'), f('body', 'Message / compte rendu', 'textarea', { required: true, wide: true }), f('status', 'Statut', 'select', { options: status(['Nouveau', 'new'], ['Lu', 'read'], ['Envoyé', 'sent'], ['Échec', 'failed'], ['Archivé', 'archived']), defaultValue: 'new' }), ...LEGACY_RESOURCE_DEFINITIONS.messages.fields.filter((field) => ['provider_message_id', 'metadata'].includes(field.name))],
    { canDelete: false }),
