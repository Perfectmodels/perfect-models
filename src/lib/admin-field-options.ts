...
const SERVICE_CATEGORY_OPTIONS = [
  option('Services Mannequinat'),
  option('Services Mode et Stylisme'),
  option('Services Événementiels'),
];
const ARTICLE_CATEGORY_OPTIONS = [
  option('Actualités'), option('Talents'), option('Événements'), option('Mode'), option('Conseils'), option('Agence'),
];
...
export function enhanceAdminFields(resource: ResourceName, fields: readonly CrudField[]): CrudField[] {
  return fields.map((field) => {
    ...
    if (resource === 'services' && field.name === 'category') {
      return { ...field, type: 'select', options: SERVICE_CATEGORY_OPTIONS };
    }
    if (resource === 'magazine' && field.name === 'category') {
      return { ...field, type: 'select', options: ARTICLE_CATEGORY_OPTIONS };
    }
    if (resource === 'gallery' && field.name === 'provider') {
      return { ...field, type: 'select', options: MEDIA_PROVIDER_OPTIONS };
    }
    if (resource === 'gallery' && field.name === 'category') {
      return { ...field, type: 'select', options: MEDIA_CATEGORY_OPTIONS };
    }
