...
function PassageLightbox({ item, initialIndex, onClose }: { item: PersonItem; initialIndex: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-4" role="dialog" aria-modal="true" aria-label={`Galerie ${item.name}`} onClick={onClose}>
      ...
      <div className="relative flex h-full w-full max-w-6xl items-center justify-center" onClick={(event) => event.stopPropagation()}>
        ...
        {images.length > 1 && <>
          <button type="button" onClick={() => setIndex((current) => (current - 1 + images.length) % images.length)} aria-label="Photo précédente" className="absolute left-1 top-1/2 -translate-y-1/2 rounded-full border border-white/20 bg-black/70 p-3 text-white hover:border-pm-gold hover:text-pm-gold"><ChevronLeftIcon className="h-6 w-6" /></button>
          <button type="button" onClick={() => setIndex((current) => (current + 1) % images.length)} aria-label="Photo suivante" className="absolute right-1 top-1/2 -translate-y-1/2 rounded-full border border-white/20 bg-black/70 p-3 text-white hover:border-pm-gold hover:text-pm-gold"><ChevronRightIcon className="h-6 w-6" /></button>
        </>}
...
function PeopleSection({ title, eyebrow, items, galleries = false }: { title: string; eyebrow: string; items: PersonItem[]; galleries?: boolean }) {
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {items.map((item, index) => (
            <article key={`${item.name}-${index}`} className="overflow-hidden rounded-2xl border border-white/10 bg-black/40">
              {item.images?.[0] ? (
                <button type="button" className="group relative block w-full" onClick={() => galleries && setLightbox({ item, index: 0 })} aria-label={galleries ? `Voir la galerie de ${item.name}` : item.name}>
                  <img src={item.images[0]} alt={item.name} className="aspect-[4/3] w-full object-cover transition duration-500 group-hover:scale-[1.02]" />
                  {galleries && <span className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 to-transparent px-4 pb-4 pt-10 text-left text-[10px] font-black uppercase tracking-[0.25em] text-white/80">Voir le passage · {item.images.length} photo{item.images.length > 1 ? 's' : ''}</span>}
                </button>
              ) : (
...
export default function FashionDayPage() {
  ...
  useEffect(() => {
    if (events.length && (selectedEdition === null || !events.some((event) => event.edition === selectedEdition))) setSelectedEdition(events[0].edition);
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-pm-dark text-pm-off-white">
      ...
      <section className="relative min-h-[88vh] overflow-hidden"><img key={cover} src={cover} alt={`Cover Perfect Fashion Day édition ${event.edition}`} className="absolute inset-0 h-full w-full object-cover" /><div className="absolute inset-0 bg-gradient-to-t from-pm-dark via-black/35 to-black/15" /><div className="relative z-10 mx-auto flex min-h-[88vh] max-w-7xl flex-col justify-end px-4 pb-12 pt-32 sm:px-6 lg:px-10"><p className="text-[10px] font-black uppercase tracking-[0.48em] text-pm-gold">Perfect Fashion Day · Édition {String(event.edition).padStart(2, '0')}</p><h1 className="mt-3 max-w-5xl font-playfair text-5xl font-black italic leading-[0.95] text-white sm:text-7xl lg:text-8xl">{event.theme}</h1><div className="mt-8 flex flex-wrap gap-5 text-sm text-white/70"><span className="inline-flex items-center gap-2"><CalendarDaysIcon className="h-5 w-5 text-pm-gold" />{formatDate(event.date)}</span><span className="inline-flex items-center gap-2"><MapPinIcon className="h-5 w-5 text-pm-gold" />{event.location || 'Lieu à confirmer'}</span></div></div></section>
      <section className="border-b border-white/5 bg-black/40 py-6"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10"><p className="mb-4 text-[9px] font-black uppercase tracking-[0.35em] text-white/30">Choisir une édition</p><div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">{events.map((item) => { const itemCover = item.coverImageUrl || item.galleryImages?.[0] || data.siteImages.fashionDayBg; const active = item.edition === event.edition; return <button key={item.edition} type="button" onClick={() => setSelectedEdition(item.edition)} className={`group relative overflow-hidden rounded-xl border text-left transition ${active ? 'border-pm-gold' : 'border-white/10 hover:border-white/30'}`}><img src={itemCover} alt={`Cover édition ${item.edition}`} className="aspect-[16/10] w-full object-cover transition duration-500 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-3"><span className={`text-[9px] font-black uppercase tracking-widest ${active ? 'text-pm-gold' : 'text-white/60'}`}>Édition {String(item.edition).padStart(2, '0')}</span><p className="truncate font-playfair text-sm font-bold text-white">{item.theme}</p></div></button>; })}</div></div></section>
      ...
      <PeopleSection title="Les Créateurs" eyebrow="Showcase" items={event.stylists ?? []} galleries={event.edition === 2} />
      <PeopleSection title="Les Artistes" eyebrow="Performances" items={event.artists ?? []} />
      {(event.featuredModels?.length ?? 0) > 0 && <section className="border-t border-white/5 bg-black/30 py-14"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10"><p className="text-[10px] font-black uppercase tracking-[0.4em] text-pm-gold/60">On the runway</p><h3 className="mt-2 font-playfair text-3xl font-black">Mannequins vedettes</h3><div className="mt-6 flex flex-wrap gap-2">{(event.featuredModels ?? []).map((name) => <span key={name} className="rounded-full border border-pm-gold/20 px-4 py-2 text-sm text-white/70">{name}</span>)}</div></div></section>}
      {(event.partners?.length ?? 0) > 0 && <section className="border-t border-white/5 py-14"><div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-10"><p className="text-[10px] font-black uppercase tracking-[0.4em] text-pm-gold/60">Partenaires</p><div className="mt-6 grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">{(event.partners ?? []).map((partner, index) => <div key={`${partner.name}-${index}`} className="rounded-xl border border-white/10 p-4"><span className="text-[9px] uppercase tracking-[0.3em] text-pm-gold/50">{partner.type || 'Partenaire'}</span><p className="mt-1 font-playfair text-lg font-bold">{partner.name}</p></div>)}</div></div></section>}
