...
export default async function ModelsAdminPage() {
  ...
  return <div className="space-y-5"><section className="flex flex-col gap-4 rounded-[1.7rem] border border-pm-ink/10 bg-white p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="control-kicker">Fiche talent 360°</p><h1 className="mt-1 font-playfair text-3xl font-semibold">Roster professionnel</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-pm-ink/50">Les mensurations sont maintenant des champs structurés et directement exploitables par la recherche et le matching. Aucun JSON technique n’est demandé à l’administrateur.</p></div><div className="flex flex-wrap gap-2"><Link href="/admin/talent-search" className="rounded-full bg-pm-ink px-5 py-2.5 text-xs font-black uppercase tracking-[.08em] text-white">Recherche avancée</Link><Link href="/admin/talent-availability" className="rounded-full border border-pm-ink/15 px-5 py-2.5 text-xs font-black uppercase tracking-[.08em]">Disponibilités</Link></div></section><PaginatedResourceManager resource="models" title="Mannequins / Talents" primaryKey="id" columns={MODEL_ADMIN_COLUMNS} fields={MODEL_ADMIN_FIELDS} initialRows={data || []} initialTotal={Number(count || 0)}/></div>;
}
