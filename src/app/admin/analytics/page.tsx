...
export default async function AnalyticsPage(){
 ...
 const talentRows: RankingRow[]=topTalents.map((row)=>({title:row.name,metrics:[['Bookings',String(row.bookings)],['Demandes',String(row.requests)],['CA',money(row.revenue)]]}));
 const clientRows: RankingRow[]=topClients.map((row)=>({title:row.name,metrics:[['Bookings',String(row.bookings)],['CA',money(row.revenue)],['À encaisser',money(row.outstanding)]]}));
