...
export default async function AdminResourceRoute({ params }: { params: Promise<{ slug: string[] }> }) {
  ...
  return <PaginatedResourceManager resource={resource} title={definition.title} primaryKey={definition.primaryKey} columns={definition.columns} fields={fields} initialRows={Array.isArray(data) ? data : []} initialTotal={Number(count || 0)} canCreate={definition.canCreate} canDelete={definition.canDelete}/>;
}
