...
function Integrations({ draft, update }: any) {
  return <Panel title="Intégrations" description="Les identifiants techniques restent masqués. Pour une clé réellement secrète côté serveur, privilégiez les variables d’environnement Vercel plutôt que la base de données."><div className="grid md:grid-cols-2 gap-6"><Secret label="Brevo API Key" value={draft.apiKeys?.brevoApiKey || ''} onChange={(v:string)=>update('apiKeys','brevoApiKey',v)} /><Field label="Email expéditeur" value={draft.apiKeys?.defaultFromEmail} onChange={(v:string)=>update('apiKeys','defaultFromEmail',v)} type="email" /><Secret label="ImgBB API Key" value={draft.apiKeys?.imgbbApiKey || ''} onChange={(v:string)=>update('apiKeys','imgbbApiKey',v)} /><Secret label="Dropbox Access Token" value={draft.apiKeys?.dropboxAccessToken || ''} onChange={(v:string)=>update('apiKeys','dropboxAccessToken',v)} /><Secret label="Gemini API Key" value={draft.apiKeys?.geminiApiKey || ''} onChange={(v:string)=>update('apiKeys','geminiApiKey',v)} /><Secret label="Firebase VAPID Key" value={draft.apiKeys?.vapidKey || ''} onChange={(v:string)=>update('apiKeys','vapidKey',v)} /><Field label="Chatbot ID" value={draft.apiKeys?.chatbotId} onChange={(v:string)=>update('apiKeys','chatbotId',v)} /></div></Panel>;
}
