...
export default async function ModelAgencyPage() {
  ...
  return <main className="min-h-screen bg-pm-ivory px-4 py-7 text-pm-ink sm:px-6 lg:px-10"><div className="mx-auto max-w-7xl space-y-6"><header className="rounded-[2rem] bg-pm-wine p-7 text-white sm:p-9"><p className="text-[10px] font-black uppercase tracking-[.2em] text-pm-gold-light">Portail mannequin · {model.name}</p><h1 className="mt-3 font-playfair text-5xl font-semibold">Ma carrière avec l’agence</h1><p className="mt-4 max-w-3xl text-sm leading-7 text-white/65">Disponibilités, castings, options, bookings, contrats, droits d’image et paiements sont regroupés dans ce centre opérationnel.</p><Link href="/profil" className="mt-5 inline-flex rounded-full border border-white/30 px-5 py-2.5 text-xs font-black uppercase tracking-[.08em]">Retour à mon profil</Link></header><section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{[["Jobs à venir",String(upcomingJobs.length)],["Castings en attente",String(pendingCastings)],["Contrats à traiter",String(pendingContracts)],["Paiements reçus",money(paymentTotal)]].map(([label,value])=><div key={label} className="rounded-[1.5rem] border border-pm-ink/10 bg-white p-5"><p className="text-[9px] font-black uppercase tracking-[.12em] text-pm-coral">{label}</p><p className="mt-4 font-playfair text-4xl font-semibold">{value}</p></div>)}</section><ModelAgencyPortal initialAvailability={availability||[]} initialCastings={castings}/><section className="grid gap-5 lg:grid-cols-2"><Panel title="Mes bookings">{(bookings||[]).map((row:any)=><Row key={row.id} title={row.title} meta={`${row.status} · ${row.starts_at?new Date(row.starts_at).toLocaleDateString('fr-FR'):'date à confirmer'}${row.model_net_amount?` · ${money(Number(row.model_net_amount),row.currency||'XAF')}`:''}`}/>)}</Panel><Panel title="Mes options">{(options||[]).map((row:any)=><Row key={row.id} title={`Option ${row.option_rank} · ${row.title}`} meta={`${new Date(row.starts_at).toLocaleDateString('fr-FR')} → ${new Date(row.ends_at).toLocaleDateString('fr-FR')}`}/>)}</Panel><Panel title="Mes contrats">{(contracts||[]).map((row:any)=><div key={row.id} className="flex items-center gap-3 border-b border-pm-ink/[.07] py-3 last:border-0"><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold">{row.title}</p><p className="mt-1 text-xs text-pm-ink/45">{row.contract_type} · {row.status}{row.expires_at?` · expire ${new Date(row.expires_at).toLocaleDateString('fr-FR')}`:''}</p></div>{row.document_url&&<a href={row.document_url} target="_blank" rel="noreferrer" className="rounded-full border border-pm-ink/10 px-3 py-2 text-xs font-bold">Ouvrir</a>}</div>)}</Panel><Panel title="Droits d’image">{(rights||[]).map((row:any)=><Row key={row.id} title={row.campaign} meta={`${row.status} · ${row.starts_on} → ${row.ends_on} · ${(row.territory||[]).join(', ')||'territoire non précisé'}`}/>)}</Panel></section></div></main>;
}
