...
export default async function Page({ params }: PageProps) {
  ...
  const schema = article ? [{
    ...
    inLanguage: 'fr-GA',
  }, breadcrumbJsonLd([{ name: 'Accueil', path: '/' }, { name: 'Blog', path: '/blog' }, { name: article.title, path }])] : null;
  return <>{schema && <JsonLd data={schema} />}<LegacyRoute component="ArticleDetail" /></>;
}
