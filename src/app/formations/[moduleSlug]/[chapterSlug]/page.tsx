    ...
    <main className="min-h-screen bg-pm-ivory px-5 py-10 text-pm-ink sm:px-8 lg:px-10">
      <article className="mx-auto max-w-[1180px] space-y-7">
        <header className="relative overflow-hidden rounded-[2.4rem] bg-pm-wine p-7 text-white sm:p-10 lg:p-12">
          <div aria-hidden="true" className="absolute -right-24 -top-28 h-80 w-80 rounded-full bg-pm-coral/40 blur-3xl" />
          <div className="relative max-w-4xl">
            ...
            <div className="mt-7 flex flex-wrap gap-2"><Badge>≈ {chapter.estimated_minutes} min</Badge><Badge>Lecture {chapter.minimum_read_percent}%</Badge><Badge>Quiz {questionRows.length} questions</Badge><Badge>Validation {chapter.pass_score}%</Badge></div>
          </div>
        </header>
        ...
        <div className="space-y-5">
          {sections.map((section, index) => <section key={`${section.title}-${index}`} className="control-card"><p className="control-kicker">Partie {String(index + 1).padStart(2, '0')}</p><h2 className="mt-2 font-playfair text-4xl font-semibold">{section.title || 'À retenir'}</h2><p className="mt-6 text-[15px] leading-8 text-pm-ink/62">{section.content || chapter.summary}</p></section>)}
        </div>
...
function Badge({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-white/15 bg-white/10 px-3 py-2 text-[8px] font-black uppercase tracking-[.14em] text-white/75">{children}</span>;
}
