...
function normalizeRole(record: Record<string, unknown>): string {
  ...
  const isDelegatedAdmin = role === 'admin' || (record.permissions && typeof record.permissions === 'object' && (record.permissions as Record<string, unknown>).isAdmin === true) || record.adminPermissions !== undefined;
...
async function resolve(identifier: string) {
  ...
  try {
    ...
    const row = all.find((item: unknown) => {
      ...
      return [r.identifier,r.matricule,r.email,r.loginEmail,r.login_email,r.username,r.name].filter(Boolean).map(v=>String(v).toLowerCase()).includes(candidate);
    });
    if (!row) return NextResponse.json({ error: 'Identifiant introuvable ou compte inactif.' }, { status: 404 });
    const r=row as Record<string,unknown>;
    const email=String(r.email||r.loginEmail||r.login_email||(String(r.matricule||r.identifier||'').toLowerCase()+'@perfectmodels.online'));
    return NextResponse.json({email,identifier:String(r.identifier||r.matricule||candidate),role:normalizeRole(r),name:String(r.name||r.displayName||candidate)},{headers:{'Cache-Control':'no-store'}});
  } catch (error) {
